...
[TestFixture]
public sealed class ResumeImportAntiforgeryFlowTests
{
    [Test]
    public async Task ParseAsync_ReturnsBadRequest_WhenAntiforgeryTokenIsMissing()
    {
        await using var host = await CreateHostAsync();
        using var client = host.GetTestClient();

        var cookies = await GetIssuedCookiesAsync(client);
        using var request = CreateParseRequest(cookies);
    ...
    [Test]
    public async Task ParseAsync_Succeeds_WhenAntiforgeryTokenIsProvided()
    {
        await using var host = await CreateHostAsync();
        using var client = host.GetTestClient();
    ...
    private static async Task<WebApplication> CreateHostAsync()
    {
        ...
        builder.Services.AddSingleton<IResumeImportService, StubResumeImportService>();
    ...
    private static HttpRequestMessage CreateParseRequest(string cookies)
    {
        var request = new HttpRequestMessage(HttpMethod.Post, "/api/admin/resume-import/parse");
